...
export const VendorForgotPasswordOTP = () => {
    ...
    const resendMutation = useMutation({
        mutationFn: async () => {
            try {
                const payload = { email };
                console.log(payload)
                const res = await axios.post("https://shopnest.zapto.org/api/auth/venodr/forgot-password-otp", payload);
                return res.data;
            } catch (error: any) {
    ...
    const otpMutation = useMutation({
        mutationFn: async (data: string) => {
            try {
                ...
                const { data: res } = await axios.post(`https://shopnest.zapto.org/api/auth/vendor/verify-password`, payload);
                return res;
            } catch (error: any) {
                const err = error.response?.data;
                if (err?.errors?.length !== 0) {
        ...
        onSuccess: () => {
            ...
            navigate({ to: `/vendor/forgot-password` })
        }
    });
