    ...
    const resendMutation = useMutation({
        mutationFn: async () => {
            try {
                const payload = { email };
                console.log(payload)
                const res = await axios.post("http://localhost:3001/auth/venodr/forgot-password-otp", payload);
                return res.data;
            } catch (error: any) {
    ...
    const otpMutation = useMutation({
        mutationFn: async (data: string) => {
            try {
                ...
                const { data: res } = await axios.post(`http://localhost:3001/auth/vendor/verify-password`, payload);
                return res;
            } catch (error: any) {
                const err = error.response?.data;
                if (err?.errors?.length !== 0) {
