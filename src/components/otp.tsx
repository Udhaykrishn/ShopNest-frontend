...
const OTPVerification = () => {
    ...
    const resendMutation = useMutation({
        mutationFn: async () => {
            try {
                const payload = { email };
                const res = await axios.post("http://shopnest.zapto.org/api/auth/user/resend-otp", payload);
                return res.data;
            } catch (error: any) {
    ...
    const otpMutation = useMutation({
        mutationFn: async (data: string) => {
            try {
                ...
                const { data: res } = await axios.post(`http://shopnest.zapto.org/api/auth/user/verify-otp`, payload);
                return res;
            } catch (error: any) {
                const err = error.response?.data;
                if (err?.errors?.length !== 0) {
